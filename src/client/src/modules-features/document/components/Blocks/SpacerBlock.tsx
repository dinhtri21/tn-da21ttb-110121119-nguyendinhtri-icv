...
export function SpacerBlock({ height = 20, onHeightChange }: SpacerBlockProps) {
  ...
  return (
    ...
    >
      ...
      <div 
        className="absolute right-2 top-1/2 transform -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
      >
        <button
      ...
      {/* Height indicator */}
      <div 
        className="absolute left-2 top-1/2 transform -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-xs text-gray-500"
      >
        {height}px
      </div>
