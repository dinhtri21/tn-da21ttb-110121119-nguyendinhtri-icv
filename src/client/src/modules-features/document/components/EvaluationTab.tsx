...
import { IEvaluates } from "@/interface/evaluate";
...
export default function EvaluationTab({ id }: EvaluationTabProps) {
  const query = useQuery<IEvaluates>({
  ...
  return (
    <div>
      <h3 className="text-sm font-medium text-gray-600">Đánh giá CV bởi AI</h3>
      {!query.data ? (
      ...
      {query.isSuccess && (
        <Table verticalSpacing="sm">
          ...
          <Table.Tbody>
            {query.data.areas.map((evaluation, index) => (
              <Table.Tr key={index}>
                <Table.Td p={4}>
